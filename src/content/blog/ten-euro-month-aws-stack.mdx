...
The S3 bucket here only holds this instance's own config and logs. To point the
rest of the homelab at it, Proxmox VMs, Docker volumes, Nextcloud,
[automated S3 and Glacier backups with restic](/homelab/aws-s3-homelab-backup/)
covers the scheduling and the real monthly cost.
...
> **Security note:** The SSH rule above allows access from any IP. For production use, replace `0.0.0.0/0` with your home IP (`curl -4 ifconfig.me`/32) or, better yet, use [Tailscale](/homelab/wireguard-tailscale-guide/) to access SSH without exposing port 22 at all.
