...
If you've been following the [homelab Ansible guide](/homelab/ansible-homelab-automation), you already know the power of describing your infrastructure in code and running a single command to make it real. Terraform does the same thing, but for a different layer.
...
- [GitHub Actions + AWS: CI/CD for Your Infrastructure](/aws/github-actions-aws-cicd) — automate `terraform apply` on every push
- The [Ansible homelab automation guide](/homelab/ansible-homelab-automation) — the configuration layer that pairs with Terraform
