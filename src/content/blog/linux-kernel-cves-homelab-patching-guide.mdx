---
title: "Mitigating 400+ Linux Kernel CVEs in Your Homelab"
...
### Worked examples from this tier list

Abstract tiers are easy to agree with and hard to apply, so here is where recent kernel bugs actually landed for me:

- **Medium, LPE, patch available:** [Fragnesia](/homelab/fragnesia-linux-privilege-escalation-homelab-patch-guide/), [Dirtyfrag](/homelab/dirtyfrag-linux-lpe-homelab-patch-guide/), [the XFS bug in CVE-2026-64600](/homelab/refluxfs-linux-kernel-xfs-lpe-cve-2026-64600-homelab-patching/), and the [AEAD socket use-after-free](/homelab/aead-socket-vulnerability-linux-kernel-patch-homelab/). All four need a local account first. Normal update cycle for a single-user box, urgent the moment you hand out shell accounts.
- **Medium, LPE, no patch yet:** [Dirty Frag](/homelab/dirty-frag-no-patch-linux-lpe-homelab-mitigation/) is the awkward case the tier list does not cover, where the only move is mitigation rather than a version bump.

Worth reading alongside all of this: [kernel CVE disclosure is genuinely broken](/homelab/linux-kernel-cve-disclosure-gap-homelab-security/), which is a large part of why the raw count hit 400+ in the first place.
...
-   **Set Up Unattended Upgrades:** Dive deeper into the configuration for automatically patching your user-space applications with our [Full Guide to Unattended-Upgrades on Debian](/homelab/debian-13-5-trixie-homelab-update-guide/).
-   **Harden Your Host:** Kernel patching is just one piece. Learn about firewall rules, user access control, and more in our [Linux Security Hardening Guide](/homelab/linux-security-hardening/).
-   **Monitor Everything:** How do you know if a patch failed or a server didn't come back up after a reboot? Set up monitoring with [Prometheus and Grafana for Your Homelab](/homelab/grafana-prometheus-homelab/).
-   **Translate to the Cloud:** The same principles of scanning and automated patching apply to the cloud. See how it works at scale with [AWS Inspector for EC2 Vulnerability Scanning](/homelab/linux-security-hardening/).