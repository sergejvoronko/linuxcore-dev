...
**Put a reverse proxy in front.** If you want HTTPS access (and you should, especially for mobile app sync), run [Traefik](/homelab/traefik-reverse-proxy) or Caddy in front and give each app a subdomain like `photos.yourdomain.com`.
...
**Monitor resource usage.** If you're running other services on the same machine, keep an eye on RAM with a [monitoring stack](/homelab/grafana-prometheus-monitoring-stack). The ML containers in Immich can compete with other memory-hungry services.
...
- **[Self-Hosted Google Photos with Immich](/homelab/immich-self-hosted-google-photos)** — full deployment guide with GPU acceleration and backup automation
- **[Docker Compose Mega-Stack](/homelab/docker-compose-mega-stack)** — deploy Immich or PhotoPrism alongside Jellyfin, Vaultwarden, and more in one stack
- **[Monitoring with Grafana & Prometheus](/homelab/grafana-prometheus-monitoring-stack)** — track resource usage across all your containers
