...
description: "A practical cost and capability comparison between running workloads on local homelab hardware vs. AWS. Real numbers, real trade-offs, and a decision framework for solo operators."
pubDate: 2026-04-14
section: "aws"
...
- **ECS/EKS** for a handful of containers (Docker Compose is fine)
- **RDS** for a single Postgres database (just run it in a container)
- **CloudWatch** for monitoring 2 servers (Prometheus is free and more capable)
- **Lambda** for workloads that run constantly (you'll pay more than a small EC2 instance)
