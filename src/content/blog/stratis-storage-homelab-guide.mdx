...
- [/homelab/zfs-proxmox-setup](/homelab/stratis-storage-homelab-guide) — Setting up ZFS pools on Proxmox for VM storage with proper ARC tuning
- [/homelab/proxmox-backup-server](/homelab/stratis-storage-homelab-guide) — Proxmox Backup Server setup with deduplication for homelab VM snapshots
- [/homelab/linux-nas-build](/homelab/stratis-storage-homelab-guide) — Building a DIY NAS on Linux with drive health monitoring and SMART alerts
- [/homelab/luks-encryption-linux](/homelab/stratis-storage-homelab-guide) — Full disk encryption on Linux servers with LUKS2 and automated unlock strategies