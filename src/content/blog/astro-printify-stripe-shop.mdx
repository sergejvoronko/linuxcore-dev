---
title: "Print-on-Demand Shop in Astro with Printify and Stripe"
description: "Add a merch shop to your Astro site using Printify for fulfilment and Stripe for payments, no inventory, ships worldwide."
pubDate: 2026-04-10
heroImage: "/images/astro-printify-stripe-shop.webp"
heroImageAlt: "Astro shop page displaying Printify print-on-demand products with Stripe checkout integration"
section: "astro"
tags:
  - astro
  - printify
...
readingTime: 14
faqs:
  - q: "Do I need to hold inventory for a print-on-demand shop?"
    a: "No. With Printify, products are printed and shipped only when a customer places an order. You never handle stock, packaging, or shipping. Your role is to set up the products, pricing, and the checkout flow, Printify handles everything physical."
  - q: "How does Printify fulfilment work?"
    a: "When an order is placed and payment is confirmed via Stripe, your Cloudflare Function sends the order to Printify's API. Printify routes it to the nearest print provider, which prints the item and ships it directly to the customer, typically within 3-7 business days."
  - q: "What fees does Stripe charge for each transaction?"
    a: "Stripe charges 1.5% + €0.25 per transaction for European cards in Europe (Stripe Pricing as of 2026). There are no monthly fees or setup costs. Stripe Radar (fraud protection) is included. Factor this into your product pricing to ensure your margin covers the fee."
  - q: "Can I add a shop to an existing Astro site without rebuilding it?"
    a: "Yes. The shop is just additional pages and a Cloudflare Pages Function endpoint. Add the shop pages to your existing src/pages directory and the checkout function to src/functions. Your existing content, layout, and styles are unaffected."
---
...
Printify            → Order submitted by your webhook, then printed and shipped
```
...
4. In Printify, go to **Sales channels** → **Add new** → **Printify API** (custom integration). This gives you the shop ID your code posts orders to
...
For orders to trigger printing and shipping, Printify needs to know about successful payments, and nothing connects the two for you. Printify has no Stripe integration: its sales channels are Etsy, Shopify, eBay, Amazon, Wix, WooCommerce and the rest, plus its own API. There is no setting that watches your Stripe account.

So this is a piece you have to build:

1. Add a Stripe webhook for `checkout.session.completed`
2. In the handler, verify the signature, then `POST /v1/shops/{shop_id}/orders.json` with the variant, quantity and the shipping address Stripe collected
3. Handle the cases that follow: a failed order call after a successful payment, address validation rejections, and refunds

Budget real time for step 3. Taking the money is the easy half; the half that matters is making sure an order exists for every payment, and that you find out when it does not.
